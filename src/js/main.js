...
const marquee = () => {
  ...
  const elMarquee = document.querySelector('.marquee__description');

  // ширины элементов
  const marqueeWrapperWidth = marqueeWrapper.offsetWidth;
  const marqueeWidth = elMarquee.scrollWidth;

  const moveMarquee = () => {
    try {
      let currentTX = getComputedStyle(elMarquee).transform.split(',');
      ...
      if (-currentTX >= marqueeWidth) {
        elMarquee.style.transform = 'translateX(' + marqueeWrapperWidth + 'px)';
      } else {
        elMarquee.style.transform = 'translateX(' + currentTX + 'px)';
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const marqueeAnimationStop = elMarquee.addEventListener('mouseover', (event) => {
    marqueeStateHandler(event);
  });

  // слушаем событие "курсор мыши убран с элемента", и вызываем обработчик событий
  const marqueeAnimationStart = elMarquee.addEventListener('mouseout', (event) => {
    marqueeStateHandler(event);
  });
};

// Проверим существование бегущей строки на странице,
// и запускаем фун-ию
if (document.querySelector('.marquee') !== null) {
  console.log('Объект найден, запускаем функцию...');
  marquee();
} else {
  console.info('Объекта несуществует!');
}
